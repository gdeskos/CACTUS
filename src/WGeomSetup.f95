subroutine WGeomSetup() 
    ...
    dsMinFS=.1 ! min grid size for free surface grid (to keep problem from getting too large)
    dsMaxFS=.5 ! max free surface grid size allowed
    dsMinW=.25  ! min grid size for wall grid (to keep problem from getting too large)
    dsMaxW=.25  ! max wall grid size allowed
    ...
    if (GPFlag==1) then
        ...
        RC=10.0 ! Clustering ratio, must be >= 1 (Ex: 10 for 10x density at center of plane)
        C1=1.0/(1.0+3.0/RC)
        C2=3.0/RC*C1
        dB=dsC/C2
    ...
    if (FSFlag==1) then
        ...
        ! Set far-field grid scale based on far-field deep water transverse wavelength for given 
        ! inflow conditions (Froude number).
        ! At low Froude number, the far-field grid scale will drop below the near field scale, 
        ! at which point, the free surface
        ! boundary condition is de-activated and replaced with a wall boundary condition. 
        ! Note that as Froude -> 0, the free 
        ! surface boundary condition becomes equivalent to the wall boundary condition...
        ! Set near-field grid scale (panel dimension over radius in the center). 
        ! If d>R use d, else use R. User can modify 
        ...
        dsOut=min(dsMaxFS,dsOut) ! GD min creates a huge problem when the froude number is very small

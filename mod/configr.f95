MODULE configr
    ...
    integer :: TimeFlag             ! Time Flag is set to 1 for Crank-Nicholson , 2 for Predictor-Corrector
    ! Params  
    ...
    integer :: iut                  ! Number of time steps over which the wake velocities are left constant (Note:     
                                    ! values of 0, -1, and -2 have defined meanings)
    ...
    integer :: ntif                 ! final number of time steps per revolution (used instead of nti during final 
                                    ! convergence step if ifc = 1)
    
    integer :: iutf                 ! final number of time steps between updating the wake velocities (used instead of 
                                    ! iut during final convergence step if ifc = 1)
    ...
    real :: convrgf                 ! Convergence level to be used for final convergence step (if ifc = 1) 
                                    ! (Note: for no convergence check, input -1)
    ...
    real :: VCRFT                   ! Vortex core radius factor (on blade discretization level) for trailing wake 
                                    ! vorticies

    real :: VCRFS                   ! Vortex core radius factor (on temporal discretization level) for spanwise wake 
                                    ! vorticies
